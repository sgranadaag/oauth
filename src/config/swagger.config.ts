...
export function setupSwagger(app: INestApplication): void {
  const config = new DocumentBuilder()
    ...
    .setDescription(
      'RFC 6749 authorization server supporting the Client Credentials, ' +
        'Resource Owner Password Credentials and Refresh Token grants. ' +
        'Only the endpoints documented here are exposed — every other route ' +
        '`oidc-provider` implements is intentionally unmounted.',
    )
    .setVersion('1.0')
    .addApiKey(
      {
