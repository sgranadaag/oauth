...
import type { SwaggerEndpointSchema } from '@interfaces/swaggerDocs.interface';

// Taken from `ApiResponse` rather than written out as `Type<unknown>`: the
// response DTOs have private constructors (construction goes through their
// static factories), which `Type` rejects and the library's own union accepts.
