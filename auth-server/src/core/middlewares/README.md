...
before routing, as distinct from `src/common/guards/` (route-level
allow/deny decisions). See `.claude/rules/architecture.md`.
...
- **Never log the `Authorization` header.** `describeCaller` decodes it
  through `decodeBasicAuth` and takes only the `id` half (the
  `client_id`); the `secret` the util hands back beside it is never
  touched here, and adding it to a log line is the one edit this file
  must never take. A bearer token is never logged either — only the
  `sub` and `client_id` claims a guard already decoded from it.
