...
// `/oauth/*` is authenticated by oidc-provider rather than by a guard, so
// nothing has annotated the request. Reading the Basic auth *username* back out
// is what keeps token requests attributable. The half after the first colon is
// the client secret and is never touched.
